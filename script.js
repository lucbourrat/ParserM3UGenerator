document.addEventListener("DOMContentLoaded", () => {
    ...
    const generateAndDownloadButton = document.getElementById(
        ...
        for (const preselectButton of preselectButtons) {
            preselectButton.addEventListener("click", (event) => {
                ...
                if (languageStates[selectedLanguage] === undefined) {
                    languageStates[selectedLanguage] = true;
                } else {
                    languageStates[selectedLanguage] =
                        !languageStates[selectedLanguage];
                }
                ...
                checkboxes.forEach((checkbox) => {
                    ...
                    if (
                        title.startsWith(`|${selectedLanguage}|`) ||
                        title.startsWith(`${selectedLanguage}|`) ||
                        title.startsWith(`|${selectedLanguage}`) ||
                        title.startsWith(`${selectedLanguage}`)
                    ) {
                        checkbox.checked = languageStates[selectedLanguage];
                    }
                });

                if (languageStates[selectedLanguage]) {
                    event.target.textContent = `Unselect ${selectedLanguage}`;
                } else {
    ...
    function setGenerateAndDownloadButton() {
        generateAndDownloadButton.addEventListener("click", () => {
            ...
            const newM3UContent = generateNewM3UContent(
                m3uContent,
                checkedTitles
            );
    ...
    function generateNewM3UContent(originalContent, titles) {
        ...
        for (let i = 1; i < originalLines.length; i = i + 2) {
            ...
            if (
                (i - 1) % 2 === 0 &&
                titles.includes(getGroupTitle(originalLines[i]))
            ) {
                includeChannel = true;
